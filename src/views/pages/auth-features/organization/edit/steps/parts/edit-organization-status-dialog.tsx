...
import type { OrganizationStatus } from "@/database/models";
import { toast } from "sonner";
import CustomTextarea from "@/components/custom-ui/textarea/CustomTextarea";

export interface EditOrganizationStatusDialogProps {
  onComplete: (organizationStatus: OrganizationStatus) => void;
}
export default function EditOrganizationStatusDialog(
  props: EditOrganizationStatusDialogProps
) {
  ...
  const add = async () => {
    try {
      ...
      // 2. Store
      const response = await axiosClient.put("statuses/modify/organization", {
      ...
      if (response.status === 200) {
        ...
        const organizationStatus = {
          id: status.organization_status_id as string,
      ...
      {storing ? (
        ...
        <CardContent className="flex flex-col mt-10 w-full md:w-[60%] gap-y-6 pb-12">
          <APICombobox
            ...
            apiUrl={"statuses/modify/organization/" + id}
            mode="single"
            cacheData={false}
          />
