...
export interface NewsTypeDialogProps {
  onComplete: (newsType: BasicModel, isEdited: boolean) => void;
  onCancel?: () => void;
  newsType?: BasicModel;
}
export default function NewsTypeDialog(props: NewsTypeDialogProps) {
  const { onComplete, newsType, onCancel } = props;
  ...
  const fetch = async () => {
    try {
      const response = await axiosClient.get(`news-types/${newsType?.id}`);
  ...
  const action = async () => {
    try {
      ...
      const response = newsType
        ? await axiosClient.put("news-types", data)
        : await axiosClient.post("news-types", data);
      if (response.status === 200) {
        toast.success(response.data.message);
        if (newsType) onComplete(response.data.type, true);
