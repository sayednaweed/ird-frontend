...
import { Edit, Eye, LockKeyholeOpen, Trash2 } from "lucide-react";
  ...
  (
    ...
    return (
      ...
      >
        ...
        <td>
          {showAction && (
            <div className="w-[110px] bg-primary py-1 rounded-full flex absolute top-1/2 transform -translate-y-1/2 ltr:right-2 rtl:left-2 justify-center gap-x-3">
              {more && (
                <div
                  onClick={async () => {
                    if (loading) return;
                ...
                >
                  <LockKeyholeOpen className="text-primary-foreground size-[18px] transition" />
                </div>
              )}
              {read && !edit && (
                <div
